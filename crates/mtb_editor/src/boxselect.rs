...
pub fn box_select(commands:       &mut Commands, 
              selectbox:          &mut Query<(&mut Transform, &mut BoxSelect)>,
              meshes:             &mut ResMut<Assets<Mesh>>,
              materials:          &mut ResMut<Assets<StandardMaterial>>,
              hoverdata:          &Res<HoverData>
            ){

  let loc = hoverdata.hovered_tile_center.unwrap();
  if selectbox.is_empty(){
    ...
    let (mut t, bs) = selectbox.get_single_mut().unwrap();
...
pub fn deselect(mut commands:     Commands,
                ...
                box_select:       Query<Entity, With<BoxSelect>>
              ){ 
  picker.reset();
  
  for ent in box_select.iter(){
