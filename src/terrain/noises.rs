...
use crate::terrain::modifiers::{Modifier, ModifierType, Modifiers};
...
pub fn apply_noise<'a>(mesh: &'a mut Mesh, nd: NoiseData, dims: &'a (f32, f32)) -> &'a Mesh {
    ...
    let mut flat_modifier: Option<Modifiers> = None;
    // if let Some(flatten)  = nd.flatten {
    //     flat_modifier = Some(flatten.to_modifiers(dims));
    // }

    for pos in v_pos.iter_mut() {
        pos[1] = noise_fn.apply(pos[0], pos[2], nd.scale)*nd.height_scale;
        if let Some(ref flat_modifier) = flat_modifier {
            pos[1] = flat_modifier.apply(&pos);
        }
